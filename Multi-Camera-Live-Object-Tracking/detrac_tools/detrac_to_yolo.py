...
class CreateDataset:
        def __init__(self, DETRAC_images, DETRAC_annots, output_train, occlusion_ratio, truncation_ratio):
        ...
        def get_sequences(self):
            ...
            return sequences
        def map_to_new_class(self,cls):
            car={'Suv','Sedan','Taxi','Van','Hatchback','MiniVan','Police'}
            pickup={'Truck-Util','Truck-Flatbed','Truck-Pickup'}
            semitruck={'Truck-Box-Large','Bus','Truck-Box-Med'}
            new_cls=cls
            if cls in car : new_cls='Car'
            if cls in pickup : new_cls='Pickup'
            if cls in semitruck : new_cls='SemiTruck'
            return new_cls
        def get_classes(self, sequences):
            '''
            Iterates through every annotation to get used class labels and output to class text file.
            '''
            file = open('detrac_classes.names', 'w')
            class_list = []
            ...
            for sequence in sequences:
                ...
                for frame in frames:
                    ...
                    if frame_num in frame_list:
                        ...
                        txt_file = open(self.output_train + txt_file_name, 'w')
        ...
        def check_train_annots(self):
            annots_list = []
            for annots_file in os.listdir(self.output_train):
                if os.path.splitext(annots_file)[1] =='.txt':
                    base = os.path.splitext(annots_file)[0]
                    annots_list.append(base)
            for img_file in os.listdir(self.output_train):
                if os.path.splitext(annots_file)[1] =='.jpg':
        ...
        def generate_train_valid_files(self):
            ...
            data_folder = self.output_train
            abs_path = os.path.dirname(os.path.abspath(data_folder))

            list_img=[img for img in os.listdir(abs_path+'/'+data_folder) if img.endswith('.jpg')==True]
            list=[item.split('.')[0] for item in list_img]

            data_train, data_valid = train_test_split(list, test_size=0.20, random_state=42)

            # Train file
            file_train = open(abs_path+'/'+'train.txt', 'w')
            for i in tqdm(range(len(data_train))):
                file_train.write(abs_path+'/'+data_folder + data_train[i] + '.jpg' + "\n")

            # Valid file
            file_valid = open(abs_path+'/'+'valid.txt', 'w')
            for j in tqdm(range(len(data_valid))):
                file_valid.write(abs_path+'/'+data_folder + data_valid[j] + '.jpg' + "\n")
...
def main():
    ...
    print('Creating Train and Valid path files ...')
    create_dataset.generate_train_valid_files()
